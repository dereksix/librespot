...
| Storage resolve | The desktop uses `storage-resolve/v2/files/audio/`; upstream librespot used the older `/storage-resolve/files/audio/interactive/` route. | Updated in this fork | Try v2 first and retain the legacy route as an automatic fallback. The PCM canary validates the returned protobuf and downstream CDN path end to end. |
